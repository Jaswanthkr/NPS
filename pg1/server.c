...
// Function designed for chat between client and server. void func(int sockfd) 
{ 
char buff[MAX]; 
int n; 
...
for (;;) { 
bzero(buff, MAX); 
// read the message from client and copy it in buffer read(sockfd, buff, sizeof(buff)); 
// print buffer which contains the client contents printf("From client: %s\t To client : ", buff); 
bzero(buff, MAX); 
n = 0; 
...
htonl(INADDR_ANY); servaddr.sin_port = htons(PORT); 
...
verification if ((listen(sockfd, 5)) != 0) { 
